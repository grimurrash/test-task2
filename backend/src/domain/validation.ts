 ...
 *
 * Запятая и точка запрещены в значении ключа — решение сэра, #182. До этого
 * F7b и F7c требовали противоположного от запятой внутри ключа: F7b считал
 * её формой повтора, F7c — законным символом. Запрет снимает противоречие
 * так же, как оно уже снято у `X-Merchant-Id` (набор `A-Za-z0-9._-` не
 * содержит запятую с самого начала): раз запятая никогда не бывает частью
 * настоящего ключа, любая запятая в значении — либо чужой ввод, либо след
 * инфраструктурной склейки дублей, и оба случая отклоняются одним и тем же
 * кодом. Заодно закрывается остаточный пробел из описания параметра
 * в контракте: за склеивающим прокси дубли раньше проходили как один ключ
 * вида «K, K» — теперь любая запятая режется здесь же, до сравнения с TTL.
 * Точка запрещена тем же решением, без отдельного технического повода.
 */
export function requireIdempotencyKey(values: string[]): string {
  ...
  if (value.includes(',') || value.includes('.'))
    throw invalidIdempotencyKey('запятая и точка запрещены в значении ключа');
  return value;
}
