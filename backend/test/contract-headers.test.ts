...
/** Ключ идемпотентности: письменность не ограничена, запятая и точка запрещены, длина — 255 (F7c, F21). */
const KEY_PROBES: Probe[] = [
  ...
  // #182 закрыт решением продукта: запятая и точка запрещены в значении
  // ключа. Пробы держим по обеим сторонам новой границы — по одной на
  // символ, плюс контрольная без него, чтобы граница проверялась печатью
  // отказа, а не только его отсутствием.
  { value: 'order-42,retry-1', why: 'запятая внутри значения — запрещена, #182' },
  { value: 'order-42.retry-1', why: 'точка внутри значения — запрещена, #182' },
  { value: 'order-42-retry-1', why: 'дефис вместо запятой/точки — разрешён' },
