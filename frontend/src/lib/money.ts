...
//
// Формат НИКОГДА не бросает (#104): вне безопасных целых toString даёт
// экспоненту («1e+21»), на которой строковый путь взрывался бы в рендере, —
// такое значение показывается как есть. Неожидаемые данные портят вид
// одной суммы, а не страницу.
const fmtInt = new Intl.NumberFormat('ru-RU')

export function formatAmount(minor: number, currency: string): string {
  if (!Number.isSafeInteger(minor)) return `${minor} ${currency}`
  const sign = minor < 0 ? '−' : ''
