...
CASES = [
    ...
    # Названная цена, закреплённая проверкой: деление С ПРОБЕЛАМИ внутри
    # встроенного кода даёт отдельный токен из слэша — он теперь путь, и это
    # отказ. Отличить его от настоящего корня надёжно нельзя, и здесь выбран
    # отказ. Кейс выше («целочисленное деление») показывает обратную сторону:
    # без пробелов выражение приходит одним куском и проходит.
    ("guard-scope.py", "деление с пробелами — названная цена, отказ",
     {"tool_name": "Bash", "tool_input": {"command": "python3 -c \"print(3600 / 60)\" > docs/out.txt"}}, "deny"),
