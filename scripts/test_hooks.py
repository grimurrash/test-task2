...
def main():
    ...
    review_pairs = [
        ...
        ("строка /** внутри кода — по-прежнему не путь",
         'python3 -c "print(\'/**\')" > docs/out.txt',
         "удаление по glob от корня — настоящая цель",
         "rm -rf /*"),
        ...
        ("docker build с контекстом внутри репозитория",
         "docker build ./backend",
         "docker build с контекстом снаружи — путь хоста",
