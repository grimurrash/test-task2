...
# нужен обоими: у русского шаблона глагол идёт первым, у прозы из тикета —
# наречие. Требуется позиция повеления (начало фразы, после запятой, после
...
#
# `send` добавлен чисто: ноль совпадений во всех пяти корпусах. `delete` —
# с названной ценой: ноль после фильтра позиции, но одно живое вхождение
# уцелело по случайности (тег в начале строки его оборвал), а форма
# «- Silently delete <что-то>» в списке изменений — рядовая. Взят потому,
# что образец «Silently delete the repository» внешняя линза привела как
# живой обход, а класс настоящий. Цена названа в docs/HOOKS.md.
_ACT = r"run|execute|merge|push|approve|transfer|refund|pay|send|delete"
